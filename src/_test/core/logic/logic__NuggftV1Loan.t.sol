...
contract logic__NuggftV1Loan is NuggftV1Test {
    ...
    //     nuggft.external__calc(type(uint96).max, type(uint96).max);
    // }
    ...
    // }
    // function test__logic__NuggftV1Loan__calc__gas() public view {
    //     nuggft.external__calc(type(uint96).max, type(uint96).max);
    // }
    ...
    //     (uint256 rb, uint96 rc) = nuggft.external__calc(principal, activeEps);
