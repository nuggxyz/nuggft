...
contract swapTest__delegate is t, NuggFatherFix {
    ...
    //     nuggft_call(frank,  delegate(epoch));
    // }

    // function test__swap__delegate__mintForOneEth() public payable {
    //     assertTrue(nuggft_call(frank, delegate(, epoch));
    // }
    ...
    //     assertTrue(nuggft_call(mac, 5 * 1delegate(0**15, epoch));

    //     revertCall_delegate(dennis, 4 * 10**13, 'E:1', epoch);

    //     assertTrue(nuggft_call(dennis, 10delegate( * 10**15, epoch));

    //     assertTrue(nuggft_call(frank, 100delegate( * 10**15, epoch));
    // }
