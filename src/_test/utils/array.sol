...
function toAddress(uint256 input) pure returns (address res) {
	return address(uint160(input));
}

library array {
	function map(uint256[] memory numbers, function(uint256) pure returns (uint256) func) internal pure returns (uint256[] memory) {
	...
	function forEach(uint256[] memory numbers, function(uint256) pure func) internal pure {
		uint256 length = numbers.length;
		uint256 i;
	...
	function to256(uint128[] memory a) internal pure returns (uint256[] memory res) {
		assembly {
			res := a
		}
	}
