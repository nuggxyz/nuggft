...
contract NuggSwap is INuggSwap, ERC721Holder, ERC1155Holder, Testable, Epochable {
    ...
    function getSwap(
        ...
        (, , , uint256 leaderData) = loadStorage(token, tokenid, swapnum, address(0));
    ...
    function _submitOffer(
        ...
        (Storage storage s, uint256 swapData, uint256 offerData, uint256 leaderData) = loadStorage(
            token,
            tokenid,
            swapnum,
            account
        ...
        // if (swapnum == 0) offerData.setEth(value);
        // uint256 end = gasleft();

        s.users[account] = offerData.setEth(value);
        if (swapnum == 0) s.datas[swapnum] = swapData.setAccount(account);
        else s.datas[swapnum - 1] = swapData.setAccount(account);
